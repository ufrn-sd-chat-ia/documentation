...
- **HTTP Interface (Spring 6, `@PostExchange`):** adicionado na Fase 8 (durante prep de apresentação) especificamente na chamada MS1→MS3 (`Ms3ValidacaoHttpInterface` + `HttpServiceProxyFactory`, ver `docs/APRESENTACAO.md` seção 12.2) — até então todos os clients HTTP eram classes concretas com `RestClient`/`WebClient` manual, sem usar esse padrão declarativo específico do Spring 6, que o professor cobra por nome. As anotações do Resilience4j continuam no `Ms3ValidacaoClient` (bean concreto), não na interface, porque o AOP delas precisa de um método interceptável num bean gerenciado — a interface HTTP cuida só da chamada declarativa.
- **Gateway restrito só ao MS1 (`chat-configs/api-gateway.yml`, `discovery.locator.include-expression`):** bug de segurança real encontrado na Fase 8 — o `discovery.locator` sem restrição gerava rota dinâmica pra *todo* serviço do Eureka, então dava pra chamar `/MS2-AI-POWERED/ia/perguntar` direto pelo Gateway, sem passar pelo `@RateLimiter`/`@CircuitBreaker` do MS1 (o MS2 não tem nenhuma proteção própria — não tem nem a dependência do Resilience4j). Corrigido com `include-expression: "serviceId=='MS1-COORDENADOR'"`, deixando o MS1 como único ponto de entrada público de verdade; MS2/MS3/mcp-server-academico continuam acessíveis normalmente, só que por dentro (chamados pelo MS1), nunca direto pelo Gateway. Impacto: os testes manuais que chamavam `/MS3-SERVERLESS/validarNota` via Gateway (`docs/TESTES-MANUAIS.md`, seção 7) foram atualizados pra chamar o MS3 direto na porta dele (8083) — isso já era um teste isolado do MS3, não fazia sentido ir pelo Gateway mesmo. O plano do JMeter (`docs/jmeter/boletim-inteligente.jmx`) não foi afetado: todos os samplers já usavam só rotas `/MS1-COORDENADOR/**`.
- **Schema GraphQL enriquecido (Fase 8, prep de apresentação):** o schema original tinha só `perguntar(mensagem, chatId): String` — um campo escalar, sem nenhuma vantagem real sobre REST. Adicionado `RespostaIA`/`MensagemChat` como tipos objeto e a query `historico(chatId)` (expõe a `ChatMemory` que o `AiChatService` já mantinha internamente, mas sem nenhuma porta de saída até então). Justificativa: com dois tipos relacionados, dá pra combinar `perguntar` + `historico` numa única requisição, cada um pedindo só os campos que interessa — o argumento clássico de GraphQL sobre REST (ver `docs/GRAPHQL.md`). Impacto controlado: só MS1/MS2 precisam reiniciar; o endpoint público `GET /perguntar` (MS1) não mudou de contrato nenhum, porque `Ms2GraphQlClient` já pede só o campo `mensagem` (`retrieve("perguntar.mensagem")`) — a mudança de tipo ficou inteiramente contida na conversa interna MS1↔MS2.
